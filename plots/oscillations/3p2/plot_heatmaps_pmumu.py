...
DATA_PATH = Path("data/oscillations/3p2/delta_pmumu_heatmap_3p2.csv")
OUTPUT_PATH = Path("figures/oscillations/3p2/delta_pmumu_heatmap_3p2_4panels.png")
...
def load_data(path: Path):
	...
	with path.open("r", newline="", encoding="utf-8") as file:
		...
		for row in reader:
			if not row:
				continue
			required = ("energy_GeV", "dm41_eV2", "dm54_eV2", "delta_P_mumu")
...
def main():
	...
	colorbar.set_label(r"$\Delta P_{\mu\mu}$", fontsize = 10)
	fig.suptitle(r"$\Delta P_{\mu\mu}=P_{\mu\mu}^{3+2}-P_{\mu\mu}^{3\nu}$ en fonction de $(E,\Delta m_{45}^2)$")
