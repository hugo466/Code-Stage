...
int run_dune_nd_fig4_source_line(const SimulationConfig *cfg) {
    ...
    fprintf(stderr,
            "DUNE ND Fig.4-like source-line: ok (point=%d, L_ND=%.6g km, z_start=%.6g m, l_dec=%.6g m, bins=%d, rows=%d, POT=%.6g, M=%.6g kt, N_targets=%.6g, scale=%.6g, max_abs_rel=%.6g, csv=%s)\n",
            ctx.theory.point_id,
            detector_baseline_km,
            source_z_start_m,
            decay_pipe_length_m,
