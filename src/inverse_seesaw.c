...
int inverse_seesaw_solve_3p1(
...

    {
        ...
        for (int i = 0; i < ISS_LIGHT_DIM; ++i) {
            result->masses_eV[i] = fabs(raw_light_eigs[i]);
            for (int a = 0; a < ISS_LIGHT_DIM; ++a) {
                result->mixing_4x4[a][i] = light_vectors[a][i];
            }
        }
    }
...

    {
        ...
        const double m4 = result->masses_eV[sterile_idx];
